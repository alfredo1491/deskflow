...
TCPSocket::JobResult SecureSocket::doWrite()
{
  ...
  if (m_pendingWriteRetry) {
    bufferSize = m_pendingWriteSize;
  } else {
    bufferSize = m_outputBuffer.getSize();
    if (bufferSize != 0) {
      m_pendingWriteBuffer.resize(static_cast<size_t>(bufferSize));
      memcpy(m_pendingWriteBuffer.data(), m_outputBuffer.peek(bufferSize), static_cast<size_t>(bufferSize));
    }
  }
  ...
  if (isSecureReady()) {
    status = secureWrite(m_pendingWriteBuffer.data(), bufferSize, bytesWrote);
    if (status > 0) {
      m_pendingWriteRetry = false;
      m_pendingWriteSize = 0;
    } else if (status < 0) {
      return Break;
    } else if (status == 0) {
      m_pendingWriteRetry = true;
      m_pendingWriteSize = bufferSize;
      return New;
    }
  } else {
    return Retry;
...
int SecureSocket::secureRead(void *buffer, int size, int &read)
{
  ...
  if (m_ssl->m_ssl != nullptr) {
    ...
    checkResult(read, m_sslReadRetry);
...
int SecureSocket::secureWrite(const void *buffer, int size, int &wrote)
{
  ...
  if (m_ssl->m_ssl != nullptr) {
    ...
    // Check result will cleanup the connection in the case of a fatal
    checkResult(wrote, m_sslWriteRetry);
...
int SecureSocket::secureAccept(int socket)
{
  ...
  checkResult(r, m_sslAcceptRetry);
...
int SecureSocket::secureConnect(int socket)
{
  ...
  checkResult(r, m_sslConnectRetry);
