...
void SettingsTests::checkStateSettingsCleanup()
{
  const auto dirtyStateFile = QStringLiteral("%1/Deskflow-dirty.state").arg(m_settingsPathTemp);
  {
    QSettings dirtyState(dirtyStateFile, QSettings::IniFormat);
    dirtyState.setValue(Settings::Gui::WindowGeometry, QRect(1, 2, 300, 200));
    dirtyState.setValue(QStringLiteral("invalid/key"), QStringLiteral("bad"));
    dirtyState.sync();
  }

  Settings::setStateFile(dirtyStateFile);

  QSettings cleanedState(dirtyStateFile, QSettings::IniFormat);
  QVERIFY(cleanedState.contains(Settings::Gui::WindowGeometry));
  QVERIFY(!cleanedState.contains(QStringLiteral("invalid/key")));
}
